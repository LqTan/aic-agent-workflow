...
class KisAgentSearchRequestSerializer(KisSearchRequestSerializer):
    top_k = serializers.IntegerField(
        required=False,
        default=12,
        min_value=1,
...
class KisAgentAttemptSerializer(serializers.Serializer):
    attempt = serializers.IntegerField(min_value=1)
    query = serializers.CharField()
    result_count = serializers.IntegerField(min_value=0)
    quality_score = serializers.FloatField(min_value=0.0, max_value=1.0)
    threshold = serializers.FloatField(min_value=0.0, max_value=1.0)
    accepted = serializers.BooleanField()


class KisAgentSearchResponseSerializer(serializers.Serializer):
    goal = serializers.CharField()
    plan = serializers.DictField()
    attempts = KisAgentAttemptSerializer(many=True)
    trace = serializers.ListField(child=serializers.DictField())
    decision = serializers.ChoiceField(choices=["accepted", "best_effort"])
    quality_score = serializers.FloatField(min_value=0.0, max_value=1.0)
    filters = serializers.DictField()
    count = serializers.IntegerField(min_value=0)
    results = KisSearchResultSerializer(many=True)
