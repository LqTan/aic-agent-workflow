...
from .serializers import (
    ApiErrorSerializer,
    KisAgentSearchRequestSerializer,
    KisAgentSearchResponseSerializer,
...
class KisAgentSearchView(APIView):
    permission_classes = [AllowAny]

    @extend_schema(
        tags=["KIS Agent"],
        summary="Agentic video retrieval with planning, evaluation and retry",
        request=KisAgentSearchRequestSerializer,
        responses={
            200: KisAgentSearchResponseSerializer,
            400: OpenApiResponse(response=ApiErrorSerializer),
            500: OpenApiResponse(response=ApiErrorSerializer),
            503: OpenApiResponse(response=ApiErrorSerializer),
        },
    ...
    def post(self, request):
        serializer = KisAgentSearchRequestSerializer(data=request.data)
        if not serializer.is_valid():
            return error_response(
                "validation_error",
                "Dữ liệu gửi lên không hợp lệ.",
        ...
        try:
            agent_result = run_agent(
                ...
                search_tool=search_kis,
            )
        except InvalidSearchRequest as exc:
            return error_response("invalid_search_request", str(exc), status.HTTP_400_BAD_REQUEST)
        except SearchServiceUnavailable as exc:
            return error_response("search_service_unavailable", str(exc), status.HTTP_503_SERVICE_UNAVAILABLE)
        except SearchServiceFailed as exc:
            return error_response("search_service_failed", str(exc), status.HTTP_500_INTERNAL_SERVER_ERROR)

        results = [format_search_result(request, item) for item in agent_result["results"]]
        response_data = {
            ...
            "filters": {
                "collection_ids": agent_result["parsed_info"]["effective_collection_ids"]
            },
            "count": len(results),
            "results": results,
        }
        response_serializer = KisAgentSearchResponseSerializer(data=response_data)
        if not response_serializer.is_valid():
            logger.error("Invalid KIS agent result: %s", response_serializer.errors)
            return error_response(
                "invalid_agent_output",
                "Kết quả Agent không đúng API contract.",
                status.HTTP_500_INTERNAL_SERVER_ERROR,
