...
def build_search_index(
    *,
    ...
    taxonomy_path = Path(
        taxonomy_path or os.getenv("KIS_DOMAIN_TAXONOMY", DEFAULT_TAXONOMY_PATH)
    ).resolve()
    ...
    try:
        ...
        metadata = {
            ...
            "pca_dimensions": int(
                np.load(temp_dir / "pca_components.npy", mmap_mode="r").shape[0]
            ),
...
def _write_object_bags_and_domains(
    ...
    for start in range(0, len(records), batch_size):
        ...
        semantic = np.clip(
            np.asarray(full_vectors[start:stop]) @ prototypes.T, -1.0, 1.0
        )
...
def _fit_and_transform_pca(
    output_dir: Path, full_vectors: np.ndarray, options: BuildOptions
) -> None:
    try:
    ...
    for start in range(0, len(full_vectors), options.transform_batch_size):
        ...
        reduced[start:stop] = pca.transform(
            np.asarray(full_vectors[start:stop], dtype=np.float32)
        )
    reduced.flush()
...
def _build_rtrees(
    ...
    try:
        for position, record in enumerate(records):
            ...
            domain_positions = np.argsort(
                -np.asarray(domain_scores[position], dtype=np.float32)
            ...
            for domain_position in domain_positions:
                ...
                trees[
                    f"collection_domain:{record.collection_id}:{domain_id}"
                ].insert(position, bounds)
                tree_counts[
                    f"collection_domain:{record.collection_id}:{domain_id}"
                ] += 1
            if (position + 1) % 20_000 == 0:
