"""CLI for building and querying the hybrid video retrieval index.

Usage examples (from the backend/ directory):

    uv run python -m app.features.search_engine.cli build-index \
        --data-root ./data/aic \
        --index-dir ./data/search_index

    uv run python -m app.features.search_engine.cli search \
        "Tìm cảnh một người đang đi xe đạp ngoài đường" --top-k 10
"""
...
from .kis import search as engine_search

DEFAULT_DATA_ROOT = Path(__file__).resolve().parent.parent.parent.parent / "data" / "aic"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m app.features.search_engine.cli",
    ...
    build.add_argument(
        "--data-root",
        type=Path,
        default=Path(os.getenv("KIS_DATA_ROOT", DEFAULT_DATA_ROOT)),
    )
    build.add_argument("--manifest", type=Path)
    build.add_argument(
        "--index-dir",
        type=Path,
        default=Path(os.getenv("KIS_INDEX_ROOT", DEFAULT_INDEX_DIR)),
    )
